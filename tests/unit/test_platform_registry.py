"""build_registry() against real Simulator/Manual out-of-process servers --
...
connection" mode to test against (see registry.py's own build_registry()
docstring): every one of these needs a real, connected PersistentIPCClient,
...
async def registry() -> AsyncIterator[list]:
    ...
    simulator_client = PersistentIPCClient(simulator_service.server.socket_path)
    manual_client = PersistentIPCClient(manual_service.server.socket_path)
    await simulator_client.start()
    await manual_client.start()
    await asyncio.sleep(0.05)  # start() doesn't wait for the connection -- give it a moment

    yield build_registry(manual_client=manual_client, simulator_client=simulator_client)

    await simulator_client.stop()
    await manual_client.stop()
...
def test_build_registry_includes_brewpi_and_tilt_when_given_state_objects():
    # BrewPi/Tilt are in-process (no IPC -- see registry.py's own
    # docstring), so unlike manual/simulator above this needs no real
    # sockets/services at all -- just something non-None at the right
    # state_attr, confirming build_registry()/PLATFORM_BINDINGS never
    # actually assumed an IPC client specifically.
    ...
    reg = build_registry(
        brewpi_connection=BrewPiConnection(clock=SimulatorClock()),
        tilt_scanner=TiltScanner(SimulatorClock(), hci_device=0),
    )
    assert {d.platform_id for d in reg} == {"brewpi", "tilt"}


def test_daemon_drivers_dispatch_generically_for_brewpi_and_tilt():
    # The same chamber_driver()/beer_temp_source()/gravity_source()
    # functions daemon/control_loop.py calls for every platform -- proof
    # this needed zero special-casing for a non-IPC platform.
    from krauken.contracts.clock import SimulatorClock
...
@pytest_asyncio.fixture
async def all_four_registry() -> AsyncIterator[list]:
    ...
    nothing about adding a real `tilt` platform touches it)."""
    from krauken.contracts.clock import SimulatorClock
    from krauken.platforms.brewpi.connection import BrewPiConnection
    from krauken.platforms.tilt.scanner import TiltScanner
...
async def test_device_id_reaches_the_tilt_driver_through_the_real_dispatch_chain():
